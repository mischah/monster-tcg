export class DeckManager {
    ...
    displayCurrentDeck() {
        ...
        // Update Deck-Count
        if (deckCountElement) {
            deckCountElement.textContent = this.game.deck.length;
        }
    ...
    updateDeckAnalysis() {
        ...
        const totalHealth = this.game.deck.reduce((sum, card) => sum + card.maxHealth, 0);
        ...
        Object.keys(rarityCount).forEach(rarity => {
            ...
            if (barElement) {
                const percentage = (rarityCount[rarity] / this.game.deck.length) * 100;
    ...
    updateDeckQualityIndicator(deckPower, deckSize) {
        ...
        const qualityElement = document.getElementById('deck-quality');
        if (qualityElement) {
